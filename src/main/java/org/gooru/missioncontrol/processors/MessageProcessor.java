...
import org.gooru.missioncontrol.processors.commands.CommandProcessorBuilder;
import org.gooru.missioncontrol.processors.exceptions.InvalidUserException;
import org.gooru.missioncontrol.processors.exceptions.VersionDeprecatedException;
...
public class MessageProcessor implements Processor {
  ...
  private ProcessorContext createContext() {
    ...
    return new ProcessorContext(userId, session, request, userIdFromURL, message.headers());
  }
  ...
  private static boolean validateUser(String userId) {
    return userId != null && (userId.equalsIgnoreCase(MessageConstants.MSG_USER_ANONYMOUS)
        || HelperUtility
        .validateUUID(userId));
  }
}
