...
public class CatalogTranscriptsSummaryProcessor extends CatalogProccessor implements MessageProcessor {

  private static final Logger LOGGER = LoggerFactory.getLogger(CatalogTranscriptsSummaryProcessor.class);

  
  public CatalogTranscriptsSummaryProcessor(Vertx vertx, Message<JsonObject> message) {
    super(vertx, message);
  }
  ...
  private void fetchSummary() {
    try {
      ...
      String resourceIds = validateAndExtractRequest(requestBody);
       ...
       this.catalogAPIService.fetchSummary(resourceIds).setHandler(asyncResult->{
